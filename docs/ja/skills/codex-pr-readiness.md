...
readiness report、commit message、PR textなどのworkflow/evidence artifactはこのskillが所有する。source、test、configuration、script、policy、documentationなどの恒久的なproduct/repository behavior correctionは `codex-implementation-loop` へ戻し、修正後にverificationとreadinessを再実行する。
...
3. diff が一つの coherent story になっているか確認する。そうでない場合は分割を提案する。
   - coherenceのために恒久file correctionが必要ならreadiness内でpatchせず、`codex-implementation-loop`へ戻す。
...
3. user が subset を選んだ場合は、選ばれた恒久修正だけを `codex-implementation-loop` へ返し、残りの comments は scope 外として記録する。
